...
export class GameAudio {
  ...
  /** User decision (2026-08-17, mirrors main's 65c275d): NPC traffic makes
      NO engine/proximity sound at all — the doppler drone pool is disabled
      outright. updateNpcs() still runs so its bookkeeping (player pose for
      the horn/chirp spatializer) stays fresh, but every voice is released
      and no per-car sound is emitted. Event one-shots (npcHorn/npcChirp)
      are unaffected. */
  ...
  updateNpcs(
    ...
    if (!GameAudio.NPC_VOICES_ENABLED) {
      // still record the listener pose — npcHorn/npcChirp spatialize off it
      this.lastPx = px;
      this.lastPz = pz;
      this.lastPh = ph;
      for (const v of this.npcVoices) {
