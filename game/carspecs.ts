...
export function testDriveSpec(spec: PhysicsSpec): PhysicsSpec {
  return {
    ...
    /* Second pass, deliberately arcade — "like No Hesi, where the cars drive
       ...
       Torque x2.7 with a 0.72 final drive: torque alone would just spin the
       ...
       would otherwise cost it. 2.7 is down from a first-pass 3.2 — full
       throttle read as too violent — and top speed cannot follow it down:
       test mode runs into physics.ts's 82 m/s sanity clamp (measured, 295.2
       km/h at both multipliers — test/testdrive-accel-sim.mjs). Keep the
       multiplier at or above 2.4 so the top-gear pull into that cap stays
       firm.
