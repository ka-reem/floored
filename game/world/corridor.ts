...
/* Steps must not overlap, and each one has to be long enough that a lane
   *centre* slides no faster than traffic can track it. The binding rate is
   ...
   A smootherstep's peak slope is 1.875·Δ/L, so a step that moves lane 1 by Δ
   needs L ≥ 35·Δ — 35 rather than the bare 33 for margin. The toll fan-out is
   the binding case: it is why the plaza is five gates wide and not six. */
const BASE_LANES = 3;
const LANE_STEPS: LaneStep[] = [
  ...
  { z0: 1270, z1: 1415, to: 5 }, // toll plaza fan-out, starting at the portal
  /* Back to the splice width, and *finished* by Z1 − DECK_EXT: the mirrors
     show the overrun behind the player, so the road has to already match its
     other end for DECK_EXT before the wrap as well as after it. */
  { z0: 1475, z1: 1615, to: 3 },
];
...
export const TOLL = { z0: 1265, z1: 1650, plazaZ0: 1420, plazaZ1: 1470 };
